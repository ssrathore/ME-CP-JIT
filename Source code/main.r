...
# Data preparation
context <- read.csv(file.path("data","context1.csv", fsep = .Platform$file.sep), row.names = 1)
...
get_context_aware_lr_perf <- function(i, correct = TRUE) {
  ...
  testing_set <- subset(all_projects, all_projects$project == project_names[i])
  print("got sets")
  tmp_context_aware_lr_model <- glmer(contains_bug ~ (0 + norm_entropy | project) + fix + ns + nf + relative_churn + lt + (1 | language) + (1 | nlanguage) + (1 | TLOC), 
                                      data = training_set,  family = "binomial", 
                                      control=glmerControl(optimizer="optimx",
                                                           optCtrl=list(method="nlminb")),nAGQ=0)
  print("trained model")
...
# Model training
print("RQ5")
context.aware <- glmer(contains_bug ~ (0 + norm_entropy | project) + fix + ns + nf + relative_churn + lt + (1 | language) + (1 | nlanguage) + (1 | TLOC), 
                       data = all_projects, family = "binomial", 
                       control=glmerControl(optimizer="optimx",
                                            optCtrl=list(method="nlminb")),nAGQ=0)
