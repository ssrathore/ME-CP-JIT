...
scale_metrics <- c("ns","nf","lt","norm_entropy")
...
for(i in 11:length(project_names)) {
  print(i)
  proj1 = p1[i]
  proj2 = p2[i]
  proj3 = p3[i]
  ...
  tmp_project_aware_rf_model <- MixRFb(training_set$contains_bug, x = 'norm_entropy+nf+rexp+loc+sexp', random = '(ns | project)', data = training_set, verbose=T, ErrorTolerance = 1, ErrorTolerance0 = 0.3, MaxIterations=20)
  tmp_project_aware_rf_pred <- predict.MixRF(tmp_project_aware_rf_model, testing_set, EstimateRE = TRUE)
  tmp_project_aware_rf_pred_corrected <- tmp_project_aware_rf_pred + median(coef(tmp_project_aware_rf_model$MixedModel)$project[,'(Intercept)']) + median(coef(tmp_project_aware_rf_model$MixedModel)$project[,'ns']) * testing_set$ns
